...
from attention_lstm import ab_lstm
...
def lstm_layer(prev, time_steps, vector_size):
    x = ab_lstm(prev, TIME_STEPS=time_steps, INPUT_DIM=vector_size,)
    x = ab_lstm(x, TIME_STEPS=time_steps, INPUT_DIM=vector_size,)
   # x = LSTM(prev, TIME_STEPS=time_steps, INPUT_DIM=vector_size,)(prev)
    #x = LSTM(x, TIME_STEPS=time_steps, INPUT_DIM=vector_size,)(x)
    return x

# def lstm_layer(prev):
#     x = LSTM(32, return_sequences=True)(prev)
#     x = BatchNormalization()(x)
#     x = Activation("relu")(x)
#     x = Conv1D(32, 1)(x)
#     x = BatchNormalization()(x)
...
def lstm_value_head(prev, time_steps, vector_size):
#     print("might need more lstm_units")
    x = ab_lstm(prev, TIME_STEPS=time_steps, INPUT_DIM=vector_size, lstm_units=1, return_seq=False)
#     print(x.shape)
#     x = Flatten()(x)
    x = Dense(32)(x)
    x = BatchNormalization()(x)
...
def lstm_policy_head(prev, time_steps, vector_size):
#     print("might need to have more lstm_units")
    x = ab_lstm(prev, TIME_STEPS=time_steps, INPUT_DIM=vector_size, lstm_units=1)
#     print(x.shape)
    x = ab_lstm(x, TIME_STEPS=time_steps, INPUT_DIM=vector_size,lstm_units=1, return_seq=False)
...
def create_net(time_steps, vector_size, num_layers=8):
...
# https://stats.stackexchange.com/questions/177905/should-i-normalize-word2vecs-word-vectors-before-using-them
    ...
    inp = Input(shape=(time_steps, vector_size))
#     x = conv_layer(x)
    x = inp
    for _ in range(num_layers):
        x = lstm_layer(x, time_steps, vector_size)
    policy = lstm_policy_head(x, time_steps, vector_size)
    value = lstm_value_head(x, time_steps, vector_size)
